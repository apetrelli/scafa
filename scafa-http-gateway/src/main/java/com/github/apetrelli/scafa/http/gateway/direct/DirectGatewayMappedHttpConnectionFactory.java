...
public class DirectGatewayMappedHttpConnectionFactory implements MappedGatewayHttpConnectionFactory {
    ...
    private Map<HostPort, GatewayHttpConnection> connectionCache = new HashMap<>();
    ...
    public void dispose(HostPort target) {
        GatewayHttpConnection connection = connectionCache.get(target);
        if (connection != null) {
    ...
    private void create(AsynchronousSocketChannel sourceChannel, HostPort hostPort, CompletionHandler<GatewayHttpConnection, Void> handler) {
        ...
        if (connection == null) {
			...
			GatewayHttpConnection newConnection = connectionFactory.create(this, sourceChannel, hostPort);
